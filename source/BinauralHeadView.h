...
public:
    ...
    ~BinauralHeadView() override {
        m_renderingContext.detach();
    }
...
private:
    ...
    {
        return juce::Matrix3D<float>::rotation (juce::Vector3D<float> (juce::degreesToRadians (m_pitch), juce::degreesToRadians (m_yaw), juce::degreesToRadians (m_roll)));
    }
    ...
    void createShaders()
    {
        ...
        std::unique_ptr<juce::OpenGLShaderProgram> newShader (new juce::OpenGLShaderProgram (m_renderingContext));
        juce::String statusText;

        if (newShader->addVertexShader (juce::OpenGLHelpers::translateVertexShaderToV3 (m_vertexShader))
            && newShader->addFragmentShader (juce::OpenGLHelpers::translateFragmentShaderToV3 (m_fragmentShader))
            && newShader->link())
        {
            m_shape.reset();
            m_attributes.reset();
    ...
    struct Attributes
    {
        ...
        void disable (juce::OpenGLContext& glContext)
        {
            ...
            if (sourceColour.get() != nullptr)
                glContext.extensions.glDisableVertexAttribArray (sourceColour->attributeID);
            if (textureCoordIn.get() != nullptr)
                glContext.extensions.glDisableVertexAttribArray (textureCoordIn->attributeID);
        }
    ...
    struct Uniforms
    {
        ...
        std::unique_ptr<juce::OpenGLShaderProgram::Uniform> projectionMatrix, viewMatrix, modelMatrix, lightPosition;
    ...
    private:
        struct VertexBuffer
        {
            VertexBuffer (juce::OpenGLContext& context, WavefrontObjFile::Shape& aShape)
