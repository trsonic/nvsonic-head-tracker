...
private:
    ...
    juce::Matrix3D<float> getModelMatrix() const
    {
        return juce::Matrix3D<float>::rotation (juce::Vector3D<float> (
            juce::degreesToRadians (m_pitch), juce::degreesToRadians (m_yaw), juce::degreesToRadians (m_roll)));
    }
        ...
        void enable (juce::OpenGLContext& glContext)
        {
            ...
            if (normal.get() != nullptr)
            {
                glContext.extensions.glVertexAttribPointer (
                    normal->attributeID, 3, GL_FLOAT, GL_FALSE, sizeof (Vertex), (GLvoid*) (sizeof (float) * 3));
            ...
            {
                glContext.extensions.glVertexAttribPointer (
                    sourceColour->attributeID, 4, GL_FLOAT, GL_FALSE, sizeof (Vertex), (GLvoid*) (sizeof (float) * 6));
            ...
            {
                glContext.extensions.glVertexAttribPointer (
                    textureCoordIn->attributeID, 2, GL_FLOAT, GL_FALSE, sizeof (Vertex), (GLvoid*) (sizeof (float) * 10));
                glContext.extensions.glEnableVertexAttribArray (
    ...
    private:
            ...
            const char* uniformName)
        {
            if (openGLContext.extensions.glGetUniformLocation (
                    shaderProgram.getProgramID(), uniformName)
                < 0)
                return nullptr;
