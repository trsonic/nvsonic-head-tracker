...
MainComponent::MainComponent()
{
    ...
    juce::StringArray rpyKeys = { "Roll, Pitch, Yaw", "Yaw, Pitch, Roll", "Pitch, Roll, Yaw", "Yaw, Roll, Pitch", "Roll, Yaw, Pitch", "Pitch, Yaw, Roll" };
    ...
    if (loadPresetXml())
    {
        ...
        {
            presets.add (presetList->getChildByAttribute ("ID", juce::String (id))->getStringAttribute ("name"));
        }
    ...
    {
        m_oscPresetCB.setTextWhenNothingSelected (juce::String ("presets.xml file is missing"));
    }
...
void MainComponent::paint (juce::Graphics& g)
{
    ...
    g.drawText ("Head Tracker OSC Bridge", 100, 0, 190, 33, juce::Justification::bottomRight);
    ...
    g.drawText ("Input Configuration", serialLabelArea.removeFromRight (270), juce::Justification::left);
    g.drawText ("IMU Orientation", imuLabelArea.removeFromRight (270), juce::Justification::left);
    g.drawText ("Output Configuration", oscLabelArea.removeFromRight (270), juce::Justification::left);
    ...
    {
        g.drawText ("Receiving port: " + juce::String (bridge.OSCinPortN), 10, 140, 280, 30, juce::Justification::centred);
        g.drawText ("OSC address: " + bridge.OSCinAddress, 10, 170, 280, 30, juce::Justification::centred);
    }
    ...
    g.setColour (clblue);
    g.drawText ("version 3.2", 10, getHeight() - 30, 280, 20, juce::Justification::bottomLeft);
    g.drawText ("2023 Tomasz Rudzki, Jacek Majer", 10, getHeight() - 30, 280, 20, juce::Justification::bottomRight);
}
...
{
    m_rollLabel.setText (juce::String (bridge.getRoll(), 1) + "°", juce::dontSendNotification);
    m_pitchLabel.setText (juce::String (bridge.getPitch(), 1) + "°", juce::dontSendNotification);
    m_yawLabel.setText (juce::String (bridge.getYaw(), 1) + "°", juce::dontSendNotification);
    m_rollOscVal.setText (juce::String (bridge.getRollOSC(), 2), juce::dontSendNotification);
    m_pitchOscVal.setText (juce::String (bridge.getPitchOSC(), 2), juce::dontSendNotification);
    m_yawOscVal.setText (juce::String (bridge.getYawOSC(), 2), juce::dontSendNotification);
...
void MainComponent::updateBridgeSettings()
{
    ...
    if (!m_quatsOscAddress.getText().startsWithChar ('/'))
        m_quatsOscAddress.setText ("/" + m_quatsOscAddress.getText(), juce::dontSendNotification);
    if (!m_rollOscAddress.getText().startsWithChar ('/'))
        m_rollOscAddress.setText ("/" + m_rollOscAddress.getText(), juce::dontSendNotification);
    if (!m_pitchOscAddress.getText().startsWithChar ('/'))
        m_pitchOscAddress.setText ("/" + m_pitchOscAddress.getText(), juce::dontSendNotification);
    if (!m_yawOscAddress.getText().startsWithChar ('/'))
        m_yawOscAddress.setText ("/" + m_yawOscAddress.getText(), juce::dontSendNotification);
    if (!m_rpyOscAddress.getText().startsWithChar ('/'))
        m_rpyOscAddress.setText ("/" + m_rpyOscAddress.getText(), juce::dontSendNotification);
    ...
    {
        juce::AlertWindow::showMessageBoxAsync (juce::AlertWindow::NoIcon, "Use the following format:", "qW, qX, -qY, qZ", "OK");
    }
...
{
    juce::StringArray qsa = juce::StringArray::fromTokens (m_quatsKeyLabel.getText(), ",", "\"");
    if (qsa.size() != 4)
    {
        return false;
    }
...
void MainComponent::loadSettings()
{
    ...
    options.folderName = juce::File::getSpecialLocation (juce::File::SpecialLocationType::currentApplicationFile).getParentDirectory().getFullPathName();
    options.storageFormat = juce::PropertiesFile::storeAsXML;
    appSettings.setStorageParameters (options);

    if (appSettings.getUserSettings()->getBoolValue ("loadSettingsFile"))
    {
        m_portListCB.setSelectedId (appSettings.getUserSettings()->getIntValue ("portListCB"), juce::dontSendNotification);
        m_quatsOscActive.setToggleState (appSettings.getUserSettings()->getBoolValue ("quatsOscActive"), juce::dontSendNotification);
        m_quatsOscAddress.setText (appSettings.getUserSettings()->getValue ("quatsOscAddress"), juce::dontSendNotification);
        m_quatsKeyLabel.setText (appSettings.getUserSettings()->getValue ("quatsKey"), juce::dontSendNotification);
        m_rollOscActive.setToggleState (appSettings.getUserSettings()->getBoolValue ("rollOscActive"), juce::dontSendNotification);
        m_pitchOscActive.setToggleState (appSettings.getUserSettings()->getBoolValue ("pitchOscActive"), juce::dontSendNotification);
        m_yawOscActive.setToggleState (appSettings.getUserSettings()->getBoolValue ("yawOscActive"), juce::dontSendNotification);
        m_rpyOscActive.setToggleState (appSettings.getUserSettings()->getBoolValue ("rpyOscActive"), juce::dontSendNotification);
        m_rollOscAddress.setText (appSettings.getUserSettings()->getValue ("rollOscAddress"), juce::dontSendNotification);
        m_pitchOscAddress.setText (appSettings.getUserSettings()->getValue ("pitchOscAddress"), juce::dontSendNotification);
        m_yawOscAddress.setText (appSettings.getUserSettings()->getValue ("yawOscAddress"), juce::dontSendNotification);
        m_rpyOscAddress.setText (appSettings.getUserSettings()->getValue ("rpyOscAddress"), juce::dontSendNotification);
        m_rollOscMin.setText (appSettings.getUserSettings()->getValue ("rollOscMin"), juce::dontSendNotification);
        m_pitchOscMin.setText (appSettings.getUserSettings()->getValue ("pitchOscMin"), juce::dontSendNotification);
        m_yawOscMin.setText (appSettings.getUserSettings()->getValue ("yawOscMin"), juce::dontSendNotification);
        m_rollOscMax.setText (appSettings.getUserSettings()->getValue ("rollOscMax"), juce::dontSendNotification);
        m_pitchOscMax.setText (appSettings.getUserSettings()->getValue ("pitchOscMax"), juce::dontSendNotification);
        m_yawOscMax.setText (appSettings.getUserSettings()->getValue ("yawOscMax"), juce::dontSendNotification);
        m_yprOrderCB.setSelectedId (appSettings.getUserSettings()->getIntValue ("yprOrderCB"), juce::dontSendNotification);
        m_ipAddress.setText (appSettings.getUserSettings()->getValue ("ipAddress"), juce::dontSendNotification);
        m_portNumber.setText (appSettings.getUserSettings()->getValue ("portNumber"), juce::dontSendNotification);
        updateBridgeSettings();
    }
    else
    {
        m_ipAddress.setText ("127.0.0.1", juce::dontSendNotification); // ip address is not stored with presets
        loadPreset (1);
    }
}
...
void MainComponent::loadPreset (int id)
{
    ...
    {
        juce::XmlElement* preset = presetList->getChildByAttribute ("ID", juce::String (id));

        m_quatsOscActive.setToggleState (preset->getBoolAttribute ("quatsOscActive"), juce::dontSendNotification);
        m_rollOscActive.setToggleState (preset->getBoolAttribute ("rollOscActive"), juce::dontSendNotification);
        m_pitchOscActive.setToggleState (preset->getBoolAttribute ("pitchOscActive"), juce::dontSendNotification);
        m_yawOscActive.setToggleState (preset->getBoolAttribute ("yawOscActive"), juce::dontSendNotification);
        m_rpyOscActive.setToggleState (preset->getBoolAttribute ("rpyOscActive"), juce::dontSendNotification);

        if (preset->getStringAttribute ("quatsOscAddress") != "")
            m_quatsOscAddress.setText (preset->getStringAttribute ("quatsOscAddress"), juce::dontSendNotification);
        if (preset->getStringAttribute ("rollOscAddress") != "")
            m_rollOscAddress.setText (preset->getStringAttribute ("rollOscAddress"), juce::dontSendNotification);
        if (preset->getStringAttribute ("pitchOscAddress") != "")
            m_pitchOscAddress.setText (preset->getStringAttribute ("pitchOscAddress"), juce::dontSendNotification);
        if (preset->getStringAttribute ("yawOscAddress") != "")
            m_yawOscAddress.setText (preset->getStringAttribute ("yawOscAddress"), juce::dontSendNotification);
        if (preset->getStringAttribute ("rpyOscAddress") != "")
            m_rpyOscAddress.setText (preset->getStringAttribute ("rpyOscAddress"), juce::dontSendNotification);
        if (preset->getStringAttribute ("quatsKey") != "")
            m_quatsKeyLabel.setText (preset->getStringAttribute ("quatsKey"), juce::dontSendNotification);
        if (preset->getStringAttribute ("yprOrderCB") != "")
            m_yprOrderCB.setSelectedId (preset->getIntAttribute ("yprOrderCB"), juce::dontSendNotification);
        if (preset->getStringAttribute ("rollOscMin") != "")
            m_rollOscMin.setText (preset->getStringAttribute ("rollOscMin"), juce::dontSendNotification);
        if (preset->getStringAttribute ("pitchOscMin") != "")
            m_pitchOscMin.setText (preset->getStringAttribute ("pitchOscMin"), juce::dontSendNotification);
        if (preset->getStringAttribute ("yawOscMin") != "")
            m_yawOscMin.setText (preset->getStringAttribute ("yawOscMin"), juce::dontSendNotification);
        if (preset->getStringAttribute ("rollOscMax") != "")
            m_rollOscMax.setText (preset->getStringAttribute ("rollOscMax"), juce::dontSendNotification);
        if (preset->getStringAttribute ("pitchOscMax") != "")
            m_pitchOscMax.setText (preset->getStringAttribute ("pitchOscMax"), juce::dontSendNotification);
        if (preset->getStringAttribute ("yawOscMax") != "")
            m_yawOscMax.setText (preset->getStringAttribute ("yawOscMax"), juce::dontSendNotification);
        if (preset->getStringAttribute ("portNumber") != "")
            m_portNumber.setText (preset->getStringAttribute ("portNumber"), juce::dontSendNotification);
    }
...
{
    juce::File dir = juce::File::getSpecialLocation (juce::File::currentApplicationFile).getParentDirectory();
