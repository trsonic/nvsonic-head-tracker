...
Bridge::Bridge() { sender.connect(m_ipAddress, m_oscPortNumber); }
...
void Bridge::disconnectOscReceiver() { disconnect(); }

void Bridge::oscMessageReceived(const juce::OSCMessage& message) {
  if (message.getAddressPattern().toString() == OSCinAddress &&
      message.size() == 4) {
...
bool Bridge::isSerialConnected() { return m_serialPortConnected; }
...
void Bridge::timerCallback() {
  if (m_serialPortConnected) {
    ...
    if (strlen(readBuffer) != 0) {
      juce::StringArray m_quatsReceived =
          juce::StringArray::fromTokens(readBuffer, ",", "\"");
...
void Bridge::pushQuaternionVector() {
  ...
  if (m_quatsActive) {
    ...
    if (m_quatsOrder.size() == 4 && m_quatsSigns.size() == 4) {
      sender.send(m_quatsOscAddress, m_quatsSigns[0] * quats[m_quatsOrder[0]],
  ...
  m_rollOSC = (float)juce::jmap(m_roll, (float)-180, (float)180, m_rollOscMin,
                                m_rollOscMax);
  m_pitchOSC = (float)juce::jmap(m_pitch, (float)-180, (float)180,
                                 m_pitchOscMin, m_pitchOscMax);
  m_yawOSC = (float)juce::jmap(m_yaw, (float)-180, (float)180, m_yawOscMin,
                               m_yawOscMax);
...
void Bridge::updateEuler() {
  ...
  // thanks to Charles Verron (https://www.noisemakers.fr/) for providing the
...
float Bridge::getRoll() { return m_roll; }

float Bridge::getPitch() { return m_pitch; }

float Bridge::getYaw() { return m_yaw; }

float Bridge::getRollOSC() { return m_rollOSC; }

float Bridge::getPitchOSC() { return m_pitchOSC; }

float Bridge::getYawOSC() { return m_yawOSC; }

void Bridge::setupQuatsOSC(bool isActive, juce::String address,
                           juce::Array<int> order, juce::Array<int> signs) {
...
void Bridge::setupRollOSC(bool isActive, juce::String address, float min,
                          float max) {
...
void Bridge::setupPitchOSC(bool isActive, juce::String address, float min,
                           float max) {
...
void Bridge::setupYawOSC(bool isActive, juce::String address, float min,
                         float max) {
...
void Bridge::setupRpyOSC(bool isActive, juce::String address,
                         juce::String key) {
